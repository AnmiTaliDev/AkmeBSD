...
void vgaprint(const char *str, int color) {
    ...
    switch (color & 0xF) {
        case 0:  fb_color = 0x00000000; break; // Black
        case 1:  fb_color = 0x000000AA; break; // Dark Blue
        case 2:  fb_color = 0x0000AA00; break; // Dark Green
        case 3:  fb_color = 0x0000AAAA; break; // Dark Cyan
        case 4:  fb_color = 0x00AA0000; break; // Dark Red
        case 5:  fb_color = 0x00AA00AA; break; // Dark Magenta
        case 6:  fb_color = 0x00AA5500; break; // Brown/Dark Yellow
        case 7:  fb_color = 0x00AAAAAA; break; // Light Gray

        case 8:  fb_color = 0x00555555; break; // Dark Gray
        case 9:  fb_color = 0x005555FF; break; // Bright Blue
        case 10: fb_color = 0x0055FF55; break; // Bright Green
        case 11: fb_color = 0x0055FFFF; break; // Bright Cyan
        case 12: fb_color = 0x00FF5555; break; // Bright Red
        case 13: fb_color = 0x00FF55FF; break; // Bright Magenta
        case 14: fb_color = 0x00FFFF55; break; // Bright Yellow
        case 15: fb_color = 0x00FFFFFF; break; // White
    }
...
void draw_logo_row(const uint32_t *logo, uint32_t logo_width, uint32_t logo_height, uint32_t spacing) {
    init_fb();
    
    if (!fb_info.initialized || !logo) return;

    uint32_t logos_per_row = fb_info.width / (logo_width + spacing);
    if (logos_per_row == 0) logos_per_row = 1;

    if (spacing == 0) {
        spacing = (fb_info.width - (logos_per_row * logo_width)) / (logos_per_row + 1);
    }

    for (uint32_t i = 0; i < logos_per_row; i++) {
        uint32_t x_pos = spacing + i * (logo_width + spacing);
        uint32_t y_pos = 0;

        for (uint32_t row = 0; row < logo_height; row++) {
            uint32_t current_y = y_pos + row;
            if (current_y >= fb_info.height) break;
            
            for (uint32_t col = 0; col < logo_width; col++) {
                uint32_t current_x = x_pos + col;
                if (current_x >= fb_info.width) break;

                uint32_t color_24bit = logo[row * logo_width + col];

                if (color_24bit == 0x000000) {
                    continue;
                }

                uint32_t color_32bit = 0xFF000000 | color_24bit;
                
                put_pixel_fb(current_x, current_y, color_32bit);
            }
        }
    }
}